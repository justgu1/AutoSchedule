...
| RLS: seller só enxerga/altera a própria concessionária; admin enxerga qualquer uma; sem contexto, nenhuma linha; só `admin`/`seller` inserem; scheduler/worker (contexto de serviço) enxergam e atualizam qualquer uma | `DealershipRlsPolicyTest` (6 casos) |
