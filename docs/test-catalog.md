...
| `GET /zip-codes/{cep}` resolve CEP → endereço via ViaCEP e cacheia (`zip_code_cache`) -- só chama o terceiro na primeira vez que aquele CEP aparece | `ZipCodeLookupServiceTest` (3 casos: cache hit, cache miss grava, CEP inexistente devolve null sem gravar) |
