...
Nunca contra o banco de dev, nem local: `phpunit.xml` força `DB_DATABASE=autoschedule_test`, um banco irmão no mesmo servidor Postgres (`bin/setup_test_database.php autoschedule_test` cria da primeira vez, `make test` migra/seeda de novo a cada rodada -- idempotente). `make e2e` usa o mesmo mecanismo com um segundo banco irmão, `autoschedule_e2e` (`bin/setup_test_database.php autoschedule_e2e`): `e2e-setup` recria `backend`/`worker`/`scheduler` apontando pra ele antes de rodar o Playwright, e `e2e` restaura pro banco de dev no final (mesmo se algum teste falhar). Sessão manual ou E2E direto contra o banco de dev deixaria `oauth_refresh_tokens`/estoque reais que quebram teste algum dia (ex: `SeederRunnerTest` resetando o admin seedado) -- isolar os dois bancos elimina essa classe de falha por completo, sem precisar resetar nada na mão. CI já era isolado por natureza (Postgres efêmero via `services:`), só o nome dos bancos foi alinhado por consistência.
